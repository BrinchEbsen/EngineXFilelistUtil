...
internal class Program
{
    ...
    private static Command MakeCreateCmd()
    {
        ...
        var outputFileArg = new Argument<FileInfo>(
            ...
            getDefaultValue: () => new FileInfo("Filelist"));
        ...
        cmd.SetHandler(CreateFilelist, inputFolderArg, outputFileArg, rootNameOption, versionOption, platformOption, splitSizeOption, scrOption);
    ...
    private static void CreateFilelist(
        ...
        FileInfo? scrFile)
    {
        if (!Directory.Exists(inputDir.FullName))
        {
            PrintError($"Directory not found: {inputDir.FullName}");
            return;
        }

        if (!Directory.Exists(outputFile.DirectoryName))
        {
            PrintError($"Cannot create filelist as output directory wasn't found: {outputFile.DirectoryName}.");
            return;
        }
        ...
        if (scrFile != null)
        {
            if (!File.Exists(scrFile.FullName))
            {
                PrintError($".scr file not found: {scrFile.FullName}");
                return;
            }
        ...
        {
            PrintError($"Error creating filelist: {e.Message}");
        }
    }
    ...
    private static void ExtractFilelist(FileInfo? binfile, DirectoryInfo outdir, FileInfo? scrFile)
    {
        if (binfile == null)
        {
            PrintError("No argument given for .bin file.");
            return;
        }

        if (!File.Exists(binfile.FullName))
        {
            PrintError($"File was not found: {binfile.FullName}.");
            return;
        }
        ...
        {
            PrintError($"Could not find/create directory {outdir.FullName}: {ex.Message}");
            return;
        }
